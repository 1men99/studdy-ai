...
class OpenAIClient:
    ...
    async def _generate(self, prompt: str, schema: type[T]) -> str:
        ...
        if self._client is None:
            self._client = httpx.AsyncClient(
                ...
                headers={
                    "Authorization": f"Bearer {settings.require_openai_key()}",
    ...
    async def generate_structured(self, prompt: str, schema: type[T]) -> T:
        ...
        is_placeholder_key = (
            ...
            or settings.OPENAI_API_KEY.startswith("your_")
            or settings.OPENAI_API_KEY.startswith("sk_placeholder")
        )
        ...
        for attempt in range(rate_limit_attempts):
            try:
                ...
                raise ValueError("OpenAI returned invalid structured JSON")
            except Exception as error:
                # If API key is rejected (401), rate limited (429), or openai fails in dev mode without mock generate_fn, return contextual fallback
